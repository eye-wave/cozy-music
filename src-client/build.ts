...
function findPages(dir: string, prefix = ''): string[] {
	...
	for (const entry of entries) {
		...
		if (stat.isDirectory()) {
			...
			const name =
				entry === 'index.ts' || entry === 'index.js' ? '' : basename(entry, extname(entry));
...
for (const route of pageRoutes) {
	let html = await serverResponse(new Request(`http://localhost${route}`)).then((res) =>
		res.text()
	);
...
const MINIFY_OPTIONS: MinifyOptions = {
	compress: { evaluate: true, unused: true, dead_code: true, passes: 3, module: true, ecma: 2020 },
	mangle: true,
