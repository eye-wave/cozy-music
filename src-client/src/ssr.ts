...
registerEnv({ van });
const { body, head, link, meta, script, h1 } = van.tags;
...
export async function serverResponse(req: Request): Promise<Response> {
	...
	else if (url.pathname.match(/\.\S+$/)) return new Response(Bun.file(__dirname + url.pathname));
	...
	try {
		...
		Page = pageModule.default ? pageModule.default() : h1('Page has no default export');
	} catch (err) {
